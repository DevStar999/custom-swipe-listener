...
public class OnHexModeSwipeListener implements OnTouchListener {
    private final GestureDetector gestureDetector;

    public OnHexModeSwipeListener(Context context) {
                    ...
                    if (Math.abs(diffY) == 0) { // User could have made a very accurate horizontal swipe
                        ...
                        if (Math.abs(diffX) == Math.abs(diffY)) { // User made an accurate diagonal swipe
                            ...
                            if (Math.abs(diffX) > Math.abs(diffY)) {
                                ...
                                if (Math.abs(diffX) > Math.abs(diffY/2)) { // Good Diagonal Swipe
                                    if (legitDiagonalSwipe) {
                                        if (diffX >= 0 && diffY >= 0) { // Right Down
                                            onSwipeRightDown();
                                        } else if (diffX >= 0 && diffY < 0) { // Right Up
                                ...
                                } else { // Not so good Diagonal Swipe
                                    if (legitHorizontalSwipe || legitVerticalSwipe) {
                                        if (diffX >= 0 && diffY >= 0) { // Right Down
                                            onSwipeRightDown();
                                        } else if (diffX >= 0 && diffY < 0) { // Right Up
                                            onSwipeRightUp();
