...
public class OnCardinalSwipeListener implements OnTouchListener {
    private final GestureDetector gestureDetector;

    public OnCardinalSwipeListener(Context context) {
    ...
    private final class GestureListener extends SimpleOnGestureListener {
        ...
        public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
            ...
            try {
                ...
                if (Math.abs(diffX) != 0 || Math.abs(diffY) != 0) { // Surely, there was a swipe gesture
                    ...
                    } else { // User made a bit inaccurate but a swipe in cardinal direction
                        if (Math.abs(diffX) >= Math.abs(diffY)) { // User most likely made a horizontal swipe
                            if (legitHorizontalSwipe) {
                                if (diffX > 0) onSwipeLeftToRight();
                                else onSwipeRightToLeft();
                                result = true;
                            }
                        } else if (Math.abs(diffX) < Math.abs(diffY)) { // User most likely made a vertical swipe
                            if (legitVerticalSwipe) {
                                if (diffY > 0) onSwipeTopToBottom();
                                else onSwipeBottomToTop();
                                result = true;
